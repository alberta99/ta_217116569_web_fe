...
export default function EnhancedTable() {
  ...
  const onDeleteClick = useCallback((salesId) => {
    axios
      .put(`${url}/salesperson/del/${salesId}`)
      .then((response) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = dataSales.map((n) => n.id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - dataSales.length) : 0;

  const visibleRows = React.useMemo(
    () =>
      stableSort(dataSales, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <button
        style={{ margin: 10 }}
        onClick={() => navigate("/admin/registersales")}
      >
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>{row.alamat_sales}</TableCell>
                    <TableCell>{row.nohp_sales}</TableCell>
                    <TableCell>{row.email_sales}</TableCell>
                    <TableCell>{row.tgl_join_sales}</TableCell>
                    <TableCell>
                      <Button
                        variant="contained"
                        onClick={() => onEditClick(row.id_sales)}
                      >
                        Edit
                      </Button>
                    </TableCell>
                    <TableCell>
                      <Button
                        variant="contained"
                        onClick={() => onDeleteClick(row.id_sales)}
                      >
                        Delete
                      </Button>
                    </TableCell>
